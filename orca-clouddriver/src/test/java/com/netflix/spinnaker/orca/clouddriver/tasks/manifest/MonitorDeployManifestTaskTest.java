...
final class MonitorDeployManifestTaskTest {
  ...
  void lookupTaskOwnerCallFails() {
    mockRetryProperties();
    when(katoService.lookupTask("2", false)).thenReturn(katoTask);
    when(katoService.lookupTaskOwner("kubernetes", "2")).thenThrow(RetrofitError.class);
  ...
  void noClouddriverOwnerPodManifestFound() {
    ...
    ImmutableMap<String, Map> operations = task.getOperations(myStage);
    assertThat(ImmutableList.of(operations)).isNotNull();
    ...
    verify(katoService).restartTask("kubernetes", "2", ImmutableList.of(operations));
  ...
  void clouddriverOwnerPodManifestFoundButForceTaskRetryFails() {
    ...
    when(katoService.lookupTask("2", false)).thenReturn(katoTask);
    when(katoService.lookupTaskOwner("kubernetes", "2")).thenReturn(taskOwner);
    when(katoService.restartTask("kubernetes", "2", ImmutableList.of(operations)))
        .thenThrow(RetrofitError.class);
  ...
  private RetrofitError retrofit404() {
